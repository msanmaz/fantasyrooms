...
export const getStaticProps = wrapper.getStaticProps(({ dispatch }) => async ({ params }) => {
	let { products, ...props } = await readCache()
	dispatch(setServerState(props))

	const { slug } = params

	const category = props.categories.find(c => c.slug === slug)
	products = products.filter(p => p.categories.findIndex(c => c.slug === slug) !== -1)
					...
					<div className='flex  flex-wrap flex-1 min-w-full'>
						...
						<div className='absolute md:top-55 top-80 w-full md:w-3/4 px-4 '>
							<div className='flex w-full' data-aos-id-blocks>
								<div className='md:px-1 px-auto space-y-2 space-x-2'></div>
							</div>
						</div>
